...
async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      // Find user by ID if provided, otherwise fetch all users
      const user = userId ? await User.findById(userId) : await User.find({});
      if (!user) {
    ...
    try {
      ...
      res.status(201).json(savedUser); // Return created user data
    } catch (err) {
      res.status(500).json({ error: err.message }); // Return error response
    }
    ...
    res.setHeader('Allow', ['GET', 'POST', 'DELETE']); // Set allowed HTTP methods in response header
